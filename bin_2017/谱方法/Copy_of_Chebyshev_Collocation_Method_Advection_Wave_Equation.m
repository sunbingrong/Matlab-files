function Copy_of_Chebyshev_Collocation_Method_Advection_Wave_Equation
...
%Error = sqrt( sum( sum( (Q - Q_exact).^2) )/(M+1)/(N+1) ) ;
...
% ================================================================
    function D_F = D_Fluxes ( Q )
        ...
        if ( mapping == 0 )
            D_F = a*D_x*F + b*(D_y*F')'             ;  % no mapping
        else
