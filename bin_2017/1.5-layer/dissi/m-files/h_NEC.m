...
for EB = 180
    figure;
    load(['../Output/orlanski_h_',num2str(EB),'E.mat']);
    nx = (EB-100)*4+1;
    ...
    h_a_max = max(max(h_a(25:66,:)));   % 126E-136E
    h_a_min = min(min(h_a(25:65,:)));   % 126E-136E
    h_a=[h_a';h_a'];
    %图锟斤拷锟斤拷锟斤拷
    [c,h] = contourf(h_a);
...
for EB = 290
    ...
    h_notice = mean_H(1:241,:);
    lon = 120:1/4:180;
    lat = 3:1/4:21;
    [X,Y]=meshgrid(lon,lat);
    m_proj('miller','lon',[120 180],'lat',[3 21]);
    m_proj('Equidistant Cylindrical','lon',[120 180],'lat',[3 21]);
