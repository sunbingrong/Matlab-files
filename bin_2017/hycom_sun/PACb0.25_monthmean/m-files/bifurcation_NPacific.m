function bifurPHI = bifurcation_NPacific(a,b,vv,topo,point)   % bj:original bifur j index  bj1:optimized bifur j index

%--------------N pacific---------------------------------------

minv=10;   % given a larger value

% b = find(v_lat>=8.75 & v_lat<=18.75);
% a = find(v_lon==126);

for j=b(1):b(end)  %(8.75-18.75~N)  it can be modified if needed
...
bj_opt=-1*b/(2*a);
% the order of bifurcation ------->>  the order of latitude
bifurPHI = bj_opt;
%--convert to lat
% bifurPHI=(bj_opt-1)*0.25+(-20);