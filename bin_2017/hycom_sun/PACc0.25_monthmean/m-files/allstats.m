% STATM Compute statistics from several series
%
% STATM = allstats(varargin)
%
...
function STATM = allstats(matrix)
...
% Check size:
if length(Cr) ~= length(Cf(1,:))
	error('Cr and Cf must be of same length');
end

% Check NaNs:
iok = find(isnan(Cr)==0 & isnan(Cf(1,:))==0);
...
for    i = 2:n
   st(i) = sqrt(sum(  (Cf(i-1,:)-mean(Cf(i-1,:)) ).^2)  / N );
end
...
for   i = 2:n
 rms(i) = sqrt(sum(  ( ( Cf(i-1,:)-mean(Cf(i-1,:)) )-( Cr-mean(Cr) )).^2)  /N);
end

%%% CORRELATIONS:
co(1)   = sum(  ( ( Cr-mean(Cr) ).*( Cr-mean(Cr) )))/N/st(1)/st(1);
for i = 2:n
co(i) = sum(  ( ( Cf(i-1,:)-mean(Cf(i-1,:)) ).*( Cr-mean(Cr) )))/N/st(i)/st(1);
end
