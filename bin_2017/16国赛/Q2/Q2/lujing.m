...
a = dlmread('Output/初估lamda/lamda.txt');
for m=1:length(filenames)
    data = textread(['Q2_data/',char(filenames(m))]);
    ...
    tic;
    lamda = a(m);
    d_simulate1 = zeros(terminal_num,base_station_num);
    d_simulate2 = zeros(terminal_num,base_station_num);
    d_simulate  = zeros(terminal_num,base_station_num);
    for i = 1:terminal_num
        for j = 1:base_station_num
            n1 = 1;n2 = 20;
            Lp = A(j,1) + B.*log10(d(i,j));
            d_simulate1(i,j) = 10^((Lp - Lp0)/(10*n1));
            d_simulate2(i,j) = 10^((Lp - Lp0)/(10*n2));
            s = sign(d_simulate1(i,j) - sqrt(lamda)*d(i,j));
            t = sign(d_simulate2(i,j) - sqrt(lamda)*d(i,j));
            x = (n1 + n2)/2;
            d_simulate(i,j) = 10^((Lp - Lp0)/(10*x));
            ...
            disp(['修正 : file = ',num2str(m),', ter_num = ', num2str(i), ', bs_num = ', num2str(j)])
        end
    end
